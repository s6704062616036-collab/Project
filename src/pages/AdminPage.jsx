                ...
                <div className="flex gap-4">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2 text-xs font-semibold">
                      ...
                      <div className={`rounded-full border px-2.5 py-1 ${bankMatchBadge(member.bankAccountNameMatchesUserName)}`}>บัญชีธนาคาร: {member.bankAccountNameMatchesUserName === true ? "ชื่อบัญชีตรงกับชื่อจริง" : member.bankAccountNameMatchesUserName === false ? "ชื่อบัญชีอาจไม่ตรงกับชื่อจริง" : member.getBankAccountNameMatchLabel?.() || "-"}</div>
                    </div>
    ...
    return (
      <div className="space-y-5">
        ...
        {!this.state.products.length ? <EmptyState message="ยังไม่พบสินค้าที่ตรงกับเงื่อนไขการค้นหา" /> : <div className="space-y-4">{this.state.products.map((product) => {
          ...
          return <article key={product.id} className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm"><div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between"><div className="flex min-w-0 gap-4"><div className="h-24 w-24 shrink-0 overflow-hidden rounded-2xl bg-zinc-100">{product.imageUrl ? <img src={product.imageUrl} alt={product.title} className="h-full w-full object-cover" /> : null}</div><div className="min-w-0 space-y-2"><div className="flex flex-wrap items-start gap-2"><div className="min-w-0 break-words text-lg font-semibold text-zinc-900 [overflow-wrap:anywhere]">{product.title || "ไม่ระบุชื่อสินค้า"}</div><div className={`shrink-0 rounded-full border px-2.5 py-1 text-xs font-semibold ${productBadge(product.status)}`}>{product.getStatusLabel()}</div><div className={`shrink-0 rounded-full border px-2.5 py-1 text-xs font-semibold ${kycBadge(product.kycStatus)}`}>{product.getKycStatusLabel()}</div></div><div className="break-words text-sm text-zinc-500 [overflow-wrap:anywhere]">ร้าน: {product.shopName || "-"} | ผู้ขาย: {product.sellerName || "-"}</div><div className="grid gap-2 text-sm text-zinc-600 md:grid-cols-2"><div>ราคา: {product.getPriceLabel()}</div><div>หมวดหมู่: {product.category || "-"}</div><div className="break-words [overflow-wrap:anywhere]">ของที่ต้องการแลก: {product.exchangeItem || "-"}</div><div>จำนวนรูป: {product.imageCount || 0} รูป</div></div>{product.description ? <div className="break-words rounded-2xl bg-zinc-50 px-3 py-2 text-sm text-zinc-700 [overflow-wrap:anywhere]">{product.description}</div> : null}</div></div><div className="grid shrink-0 gap-2 xl:w-56"><button type="button" className="rounded-2xl border border-red-200 bg-red-50 px-4 py-2.5 text-sm font-semibold text-red-700 disabled:opacity-50" disabled={isSubmitting} onClick={() => this.deleteProduct(product)}>ลบสินค้าออกจากระบบ</button></div></div></article>;
        })}</div>}
      </div>
    );
  }
